...
Once a sequence number $i$ has been selected (Step 2), and $U$ has been computed, interval mapping is applied to map $i$ into its constituent pieces representing the final sequence. The mapped value from each range identifies a specific permutation or combination for that aspect of the sequence. Using these values and methods for constructing indexed permutations and combinations, the bulk of the trial sequence can be assembled.


\subsection{Mapping Selected Values to Sequences}

Once a value has been produced for each interval, the value must be converted into its associated permutation or combination in the original design. The first value identifies a particular permutation of the crossing, the next $l$ values identify a particular source combination for the $l^{th}$ trial in the sequence\footnote{Ordering is important here due to the association between valid source combinations and specific sequences in the crossing. The same permutation that is applied to the crossing must also be applied to the set of source combinations to preserve the integrity of the association.}, and the last $|\overline{C}_{B_I}|$ values identify particular $l$-combinations to be used for each independent basic factor in the sequence. Each of these three types of values span a different dimension of the final sequence. Figure \ref{fig:value_dimensions} gives a visual representation to assist the reader.

\begin{figure}[htb]
\centering
\centerline{\includegraphics[origin=c,width=12cm]{../figures/todo.jpg}} %TODO
\caption{Dimensionality of Interval Values}
\label{fig:value_dimensions}
\end{figure}

The selected permutation value is converted to a unique inversion sequence using interval mapping. The inversion sequence is then used to construct the associated permutation. This permutation of integers is then converted into a seqeuence of level values for factors in the crossing by using each integer as an index into the sequences in $X$.

The selected values for independent basic factors are converted in similar fashion. Each selected value identifies a combination index, which is converted to a set of values using interval mapping. Each value is an index into the list of levels for the associated factor. % TODO - does this need more?
...
Levels for factors in $C_B$ and $C_D$ have now been selected. The only remaining factor is \texttt{text}, which is in $\overline{C}_{B_S}$. The value selections from $R$ for this factor were $\{0, 1, 0, 1, 0, 0\}$. We then use the values from the permutation as indices to select both the source combination list, and the selected value from that list, for each trial. In chapter 4, the set of source combination lists was established:
...
\begin{table}[htb]
  \centering
  \caption{Example - $2590^{th}$ Trial Sequence}
\begin{tabular}{rccc}
...
Generating a large number of samples can still take a significant amount of time. However, because each sample can be generated fully independently, this algorithm can be easily parallelized for a linear speedup in the number of available cores. This will be discussed further in Chapter 6.
